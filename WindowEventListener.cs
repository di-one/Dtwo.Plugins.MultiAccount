...
{
    // Not working in app service (electron.net is an app service)
    // Todo : Create console app routine for this, and compute the result in data struct
    public class WindowEventListener : IDisposable
    {
        #region PInvoke
        [DllImport("user32.dll")]
        static extern IntPtr SetWinEventHook(
                WinEvents eventMin, WinEvents eventMax, IntPtr
        ...
        private static WinEventDelegate m_focusCallback;
        private static WinEventMessageDispatcher m_dispatcher;
...
*/
        ...
        * The range of event constant values reserved for UI Automation property-changed event identifiers. * For more information, see Allocation of WinEvent IDs. */
        ...
        private Action<IntPtr> m_windowFocusCallback;
        private IntPtr m_hweHook = IntPtr.Zero;
        private bool disposedValue;

        private bool m_isStarted;

        public static IntPtr LastForegroundWindow = IntPtr.Zero;

        public void Listen(Action<IntPtr> windowFocusCallback)
        {
            try
            {
                if (m_isStarted)
                {
                    LogManager.LogWarning(
                            $"{nameof(WindowEventListener)}.{nameof(Listen)}", 
                ...
                m_dispatcher = new WinEventMessageDispatcher();
                m_dispatcher.Start(
                () => // start
                {
                    m_focusCallback = new WinEventDelegate(OnWindowFocus);
                    m_windowFocusCallback = windowFocusCallback;

                    m_hweHook = SetWinEventHook(
                        WinEvents.EVENT_SYSTEM_FOREGROUND, WinEvents.EVENT_SYSTEM_FOREGROUND,
                        IntPtr.Zero, m_focusCallback, 0, 0,
                        WinEventFlags.WINEVENT_OUTOFCONTEXT);
                },
                () => // stop
                {
                    if (m_hweHook != IntPtr.Zero)
                    {
                        if (UnhookWinEvent(m_hweHook) == false)
                        {
                            LogManager.LogWarning(
                            $"{nameof(WindowEventListener)}.{nameof(Listen)}", 
        ...
        private void OnWindowFocus(IntPtr hWinEventHook, uint eventType, IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
           // Console.WriteLine("On window focus ?");

            //WinEvents wevent = (WinEvents)hWinEventHook;

            //if (wevent == WinEvents.EVENT_SYSTEM_FOREGROUND)
            //{
                LastForegroundWindow = hwnd;
                m_windowFocusCallback?.Invoke(hwnd); // le callback est sencé être null, mais ....
            //}
        }
